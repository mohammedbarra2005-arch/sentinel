...
def build_graph_from_accounts() -> Data:
    """Construit le graphe GAT à partir du dataset SYNTHÉTIQUE ENRICHI (comptes
    individuels + vrais patterns de fraude en réseau), généré par
    generate_synthetic_accounts.py.

    Différence avec build_graph() (ci-dessus) :
      - build_graph()              -> 14 nœuds (BIC/banques), étiquette PROXY (taux de rejet)
      - build_graph_from_accounts() -> 400 nœuds (comptes individuels), VRAIE étiquette
        (on sait exactement quels comptes appartiennent à un anneau de fraude ou un
        schéma de collecte, car on les a nous-mêmes injectés lors de la génération).

    C'est cette version qui permet au GAT de démontrer sa vraie valeur ajoutée :
    détecter des patterns structurels (cycles, fan-in) invisibles pour un modèle
    qui ne regarde que des transactions isolées comme XGBoost.
    """
    accounts_path = PROCESSED_DIR / "gat_accounts_nodes.parquet"
    tx_path = PROCESSED_DIR / "gat_accounts_transactions.parquet"

    if not accounts_path.exists() or not tx_path.exists():
        raise FileNotFoundError(
            "Données de comptes synthétiques introuvables. Lance d'abord :\n"
            "  python ml_gat/generate_synthetic_accounts.py"
        )

    accounts = pd.read_parquet(accounts_path)
    tx = pd.read_parquet(tx_path)

    # Features par compte : statistiques agrégées sur ses transactions (envoyées/reçues)
    sent = tx.groupby("source").agg(
        montant_envoye_total=("amount", "sum"),
        nb_envois=("message_id", "count"),
        montant_envoye_moyen=("amount", "mean"),
    ).reset_index().rename(columns={"source": "account_id"})

    received = tx.groupby("target").agg(
        montant_recu_total=("amount", "sum"),
        nb_receptions=("message_id", "count"),
        montant_recu_moyen=("amount", "mean"),
    ).reset_index().rename(columns={"target": "account_id"})

    nodes = accounts.merge(sent, on="account_id", how="left").merge(received, on="account_id", how="left")
    nodes = nodes.fillna(0.0)

    account_to_idx = {acc: i for i, acc in enumerate(nodes["account_id"])}

    feature_cols = [
        "montant_envoye_total", "nb_envois", "montant_envoye_moyen",
        "montant_recu_total", "nb_receptions", "montant_recu_moyen",
    ]
    x = nodes[feature_cols].values.astype(np.float32)
    x = (x - x.min(axis=0)) / (x.max(axis=0) - x.min(axis=0) + 1e-8)
    x = torch.tensor(x, dtype=torch.float)
    ...
    src = tx["source"].map(account_to_idx).values
    dst = tx["target"].map(account_to_idx).values
    edge_index = torch.tensor(np.array([src, dst]), dtype=torch.long)

    data = Data(x=x, edge_index=edge_index, y=y)
    data.bic_list = nodes["account_id"].tolist()  # même nom d'attribut que build_graph(), pour compatibilité
