...
class SimpleGAT(torch.nn.Module):
    def __init__(self, in_channels: int, hidden_channels: int = 16, out_channels: int = 2,
                 heads: int = 4, dropout: float = 0.2):
        """
        in_channels  : nombre de features par noeud (6 avec le graphe enrichi, cf. graph_builder.py)
        ...
        dropout      : probabilité de dropout (régularisation), appliqué UNIQUEMENT entre
                       les deux couches -- PAS sur les features d'entrée brutes, qui sont
                       déjà peu nombreuses (6) : leur appliquer du dropout est destructeur
                       et empêchait le modèle d'apprendre (constaté empiriquement).
        """
        super().__init__()
